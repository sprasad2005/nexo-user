...
export function AdminSidebar({
  ...
  const sidebarContent = (isMobileDrawer = false) => (
    <div className="flex flex-col justify-between h-full bg-surface select-none font-sans overflow-hidden">
      <div className="flex flex-col min-h-0 flex-1 overflow-y-auto">
        ...
        <div className="h-14 px-4 border-b border-line flex items-center justify-between shrink-0 bg-surface sticky top-0 z-10">
          <div className="flex items-center gap-1.5">
            ...
            <span className="ml-1 px-1.5 py-0.5 rounded text-[10px] font-mono font-bold bg-accent/10 text-accent border border-accent/20">
              ADMIN
            </span>
          </div>
            ...
            {mainNav.map((item) => {
              ...
              return (
                <button
                  ...
                  className={`w-full h-10 flex items-center gap-3 px-3 rounded-xl text-xs font-bold transition-all cursor-pointer ${
                    isActive
                ...
                >
                  <Icon size={18} className={isActive ? "text-white dark:text-[#6B93FF]" : "text-slate-400 dark:text-[#858D99]"} />
  ...
  return (
    <>
      {/* DESKTOP SIDEBAR - FIXED 240px, ONLY VISIBLE ON lg AND ABOVE */}
      <aside className="hidden lg:flex w-[240px] bg-surface border-r border-line flex-col justify-between h-screen sticky top-0 shrink-0 select-none font-sans z-30 overflow-hidden">
        {sidebarContent(false)}
      </aside>

      {/* MOBILE SIDEBAR DRAWER - OVERLAY ON SCREENS BELOW lg */}
      {isOpen && (
        <div className="fixed inset-0 z-50 lg:hidden flex">
          ...
          />
          {/* Drawer Panel */}
          <div className="relative w-[280px] max-w-[85vw] h-full bg-surface border-r border-line z-50 shadow-2xl flex flex-col animate-in slide-in-from-left duration-200">
            {sidebarContent(true)}
          </div>
        </div>
      )}
